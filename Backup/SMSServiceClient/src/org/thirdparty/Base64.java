...
public final class Base64 {
    ...
    public static byte[] decode(String encoded) throws Base64Exception {
        ...
        if (len % FOURBYTE != 0) {
            throw new Base64Exception("decoding.divisible.four");
        }
        ...
        for (; i < numberQuadruple - 1; i++) {
            ...
            if (!isData((d1 = base64Data[dataIndex++]))
                ...
                throw new Base64Exception("decoding.general");// if found
                                                                  // "no data"
                                                                  // just return
                                                                  // null
            }
        ...
        if (!isData((d3)) || !isData((d4))) {// Check if they are PAD characters
            if (isPad(d3) && isPad(d4)) { // Two PAD e.g. 3c[Pad][Pad]
                if ((b2 & 0xf) != 0) {// last 4 bits should be zero
                    throw new Base64Exception("decoding.general");
                }
                ...
                throw new Base64Exception("decoding.general");// an error
                                                                  // like
                                                                  // "3c[Pad]r",
                                                                  // "3cdX",
                                                                  // "3cXd",
